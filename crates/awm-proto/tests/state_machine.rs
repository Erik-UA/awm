...
fn terminal_states_absorb_everything() {
    let events = [
        ...
        AgentEvent::ToolStarted { name: "Edit".into(), summary: "f.txt".into(), input: None },
        approval(),
...
fn thinking_and_tool_keep_agent_working() {
    ...
    assert_eq!(
        AgentState::Working.apply(&AgentEvent::ToolStarted { name: "Read".into(), summary: "f.txt".into(), input: None }),
        AgentState::Working
    );
}
...
fn arb_event() -> impl Strategy<Value = AgentEvent> {
    prop_oneof![
        ...
        Just(AgentEvent::ToolStarted { name: "T".into(), summary: "s".into(), input: None }),
        Just(approval()),
